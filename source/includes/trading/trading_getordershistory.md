...
| OrderId               | **long integer**. ID for the order.Can be used to filter results. Can be used to filter for a specific orderid. There is a specific API call **GetOrderHistoryByOrderId** for filtering orders by **OrderId**._optional._                                                                             |
...
| StartTimestamp        | **long integer**. Date and time at which to begin the orders history, in POSIX format. Can be used to filter results._optional._                                                                                                                                                                      |
...
| StartIndex            | **integer**. The starting index into the order history, from 0 (the most recent trade) and moving backwards in time. If not specified, defaults to 0. Can be used to filter results or for pagination._optional._                                                                                     |
